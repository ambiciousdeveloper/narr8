...
- **에이전트**: \`agents/chamber-2-4-scribe.ts\` - CreativeScribe.synthesize
`;

async function updateVolumeSOP() {
    console.log('\n🚀 Updating VOLUME_CONTROL_POLICY to v6.2...\n');

    const { error } = await supabase
        .from('agent_sop_registry')
        .update({ instruction: updatedInstruction, last_updated: new Date().toISOString() })
        .eq('sop_type', 'VOLUME_CONTROL_POLICY');

    if (error) {
        console.error('❌ Failed to update SOP:', error.message);
    } else {
        console.log('✅ Successfully updated VOLUME_CONTROL_POLICY to include Multi-Language Ratios.');
    }
}
